...
BOT_ID = os.getenv("BOT_ID", "DUMMY-BOT-new-with-x-path")
...
def open_customers_page(page):
    ...
    try:
        # XPath to find Customers link/button
        page.locator("xpath=//button[contains(text(), 'Customers')] | //a[contains(text(), 'Customers')] | //*[@role='link' and contains(text(), 'Customers')]").click()
    except Exception:
...
def fill_customer_onboarding_form(page, customer_data: dict) -> dict:
    ...
    # XPath for Full Name field
    customer_fullname = require_locator(page, "xpath=//*[@id='customer-fullname'] | //input[@id='customer-fullname']")
    customer_fullname.click()
    customer_fullname.fill("")
    customer_fullname.type(fullname_value, delay=100)
    page.wait_for_timeout(200)

    # XPath for Segment dropdown
    customer_segment = require_locator(page, "xpath=//*[@id='customer-segment'] | //select[@id='customer-segment']")
    customer_segment.select_option(label=segment_value)

    # XPath for Customer ID field
    customer_id = require_locator(page, "xpath=//*[@id='customer-id'] | //input[@id='customer-id']")
    customer_id.click()
    customer_id.fill("")
    customer_id.type(customer_id_value, delay=100)
    page.wait_for_timeout(200)

    # XPath for Gender radio button
    gender_xpath = f"xpath=//input[@name='customer-gender'][@value='{gender_value}'] | //input[@name='customer-gender' and @value='{gender_value}']"
    customer_gender = require_locator(page, gender_xpath)
    customer_gender.check()

    # XPath for Assigned Branch field
    assigned_branch = require_locator(page, "xpath=//*[@id='assigned-branch'] | //input[@id='assigned-branch']")
    ...
    # XPath for Notes field
    customer_notes = require_locator(page, "xpath=//*[@id='customer-notes'] | //textarea[@id='customer-notes']")
...
def submit_customer_onboarding(page, customer_data: dict) -> None:
    ...
    # XPath for Submit KYC button
    submit_kyc = require_locator(page, "xpath=//*[@id='submit-kyc'] | //button[@id='submit-kyc']")
    submit_kyc.click()
...
def run():
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                # XPath for Admin username input
                username_input = require_locator(page, "xpath=//input[@placeholder='admin']")
                username_input.fill("admin")

                # XPath for Password input
                password_input = require_locator(page, "xpath=//input[@placeholder='admin123']")
                password_input.fill("admin123")

                # XPath for Submit button
                login_button = require_locator(page, "xpath=//button[@type='submit']")
                login_button.click()
